...
//  ImGuiPlatformIO interface. 2026-04-21: Added a Win32-specific implementation
//  of ImGui_ImplGlfw_GetContentScaleXXXX functions for legacy GLFW 3.2.
...
//  not accessible. (#9225) 2026-01-25: [Docking] Improve workarounds for cases
//  where GLFW is unable to provide any reliable monitor info. Preserve existing
//  monitor list when none of the new one is valid. (#9195, #7902, #5683)
//  2026-01-18: [Docking] Dynamically load X11 functions to avoid -lx11 linking
//  requirement introduced on 2025-09-10. 2025-12-12: Added
//  IMGUI_IMPL_GLFW_DISABLE_X11 / IMGUI_IMPL_GLFW_DISABLE_WAYLAND to forcefully
//  disable either. 2025-12-10: Avoid repeated
//  glfwSetCursor()/glfwSetInputMode() calls when unnecessary. Lowers overhead
//  for very high framerates (e.g. 10k+ FPS). 2025-11-06: Lower minimum
//  requirement to GLFW 3.0. Though a recent version e.g GLFW 3.4 is highly
//  recommended. 2025-09-18: Call platform_io.ClearPlatformHandlers() on
...
#define GLFW_HAS_GETPLATFORM                                                   \
  ((GLFW_VERSION_COMBINED >= 3400) && defined(GLFW_PLATFORM_WAYLAND)) // 3.4+ glfwGetPlatform()
...
static void ImGui_ImplGlfw_UpdateMonitors() {
  ...
  for (int n = 0; n < monitors_count; n++) {
    ...
    // Happens on macOS sleeping (#5683) and seemingly occasionally on Windows
    // (#9195)
    if (!updated_monitors)
...
// ImGui_ImplWin32_GetDpiScaleForMonitor(). This is merely a band-aid to make
// using GLFW 3.2 a little bit nicer, but prefer to use GLFW 3.3+ or the full
// correct functions from the Win32 backend.
...
static void glfwGetWindowContentScale(GLFWwindow *, float *x_scale,
                                      float *y_scale) {
  *x_scale = *y_scale = ImGui_ImplWin32_GetLegacyDpiScale();
}
static void glfwGetMonitorContentScale(GLFWmonitor *, float *x_scale,
                                       float *y_scale) {
  *x_scale = *y_scale = ImGui_ImplWin32_GetLegacyDpiScale();
}
